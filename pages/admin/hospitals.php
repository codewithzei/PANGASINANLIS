...
include '../../includes/fetch_hospitals_data.php'; // Include the fetch file
...
<div class="">
    ...
    <div class="flex-1 md:ml-64 min-h-screen">
        ...
        <main class="p-3 sm:p-6">
            ...
            <?php if (isset($_SESSION['hospital_status'])): ?>
                <div id="globalMessage" class="fixed top-5 left-1/2 -translate-x-1/2 z-9999 w-max max-w-md animate-fade-in shadow-lg rounded-lg" style="<?php echo $_SESSION['hospital_status'] === 'success' ? 'background-color: rgb(220, 252, 231); color: rgb(20, 83, 45); border: 1px solid rgb(167, 243, 208);' : 'background-color: rgb(254, 226, 226); color: rgb(127, 29, 29); border: 1px solid rgb(252, 165, 165);'; ?>">
                    <div class="flex items-center gap-3 px-6 py-4">
                        <?php if ($_SESSION['hospital_status'] === 'success'): ?>
                        ...
                        <?php endif; ?>
                        <span class="text-sm font-medium"><?php echo htmlspecialchars($_SESSION['hospital_message']); ?></span>
                ...
                unset($_SESSION['hospital_status']);
                unset($_SESSION['hospital_message']);
                ?>
            <?php endif; ?>
            ...
            <!-- Header with Title -->
                <div class="p-6 border-b border-slate-200">
                    <div class="flex flex-col gap-4">
                        <div>
                            <h2 class="text-xl font-bold text-gray-800">Hospitals</h2>
                            <p class="text-sm text-gray-600 mt-1">Manage and configure hospitals</p>
                        </div>
                        ...
                        <div class="flex items-center gap-3 w-full">
                            ...
                            <button onclick="openModal('hospital-add-modal')"
                    ...
                    <table id="usersTable" class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-50">
                            <tr>
                                ...
                                <th scope="col" class="px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider cursor-pointer" onclick="sortTable(2)">
                                    <div class="flex items-center gap-2">
                                        Status
                            ...
                            <?php if (isset($hospitals['error'])): ?>
                                <tr>
                                    <td colspan="4" class="px-6 py-8 text-center text-red-600">
                                        <?php echo htmlspecialchars($hospitals['error']); ?>
                                    </td>
                                </tr>
                            <?php elseif (empty($hospitals)): ?>
                                ...
                                <?php foreach ($hospitals as $hospital): ?>
                                    <tr class="hover:bg-slate-50 transition-colors duration-150">
                                        ...
                                        <td class="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap">
                                            <?php 
                                            $statusClass = $hospital['status'] === 'active' 
                                                ? 'bg-green-100 text-green-800' 
                                                : 'bg-gray-200 text-gray-800';
                                            ?>
                                            <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full <?php echo $statusClass; ?>">
                                                <?php echo htmlspecialchars(ucfirst($hospital['status'])); ?>
                                            </span>
                                        </td>
                                        <td class="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex items-center gap-2">
                                                <button onclick="editHospital(<?php echo $hospital['hospital_id']; ?>)" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-blue-600 bg-blue-100 hover:bg-blue-200 rounded transition duration-150 cursor-pointer">
                                                ...
                                                <button
                                                    onclick="deleteHospital(<?php echo $hospital['hospital_id']; ?>, '<?php echo htmlspecialchars($hospital['hospital_name']); ?>')"
...
<?php include 'modals/add_hospital.php'; ?>
<?php include 'modals/edit_hospital.php'; ?>
<?php include 'modals/delete_hospital.php'; ?>
