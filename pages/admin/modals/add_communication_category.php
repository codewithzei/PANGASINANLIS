...
<div id="communication-category-add-modal" class="hidden">
    
    <!-- Modal Overlay -->
    <div 
        id="modalOverlay" 
        onclick="closeModal('communication-category-add-modal')"
    ...
    <div 
        ...
        <div class="flex items-center justify-between p-5 border-b border-gray-200">
            <h2 class="text-xl font-semibold text-gray-800">Add Communication Category</h2>
            <button 
                id="closeBtn"
                onclick="closeModal('communication-category-add-modal')"
        ...
        <div id="communicationCategoryMessage" class="hidden mx-6 mt-4 p-4 rounded-lg text-sm font-medium">
            <div id="communicationCategoryMessageContent"></div>
        </div>

        <!-- Modal Body -->
        <form id="communicationCategoryForm" class="p-6" method="POST" action="../../includes/actions_admin/add_communication_category.php">
            <div class="mb-4">
                <label for="communicationCategoryInput" class="block text-sm font-medium text-gray-700 mb-2">
                    Communication Category Name
                </label>
                <input 
                    type="text" 
                    id="communicationCategoryInput"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const modalId = 'communication-category-add-modal';
    ...
    window.openModal = function(id) {
        if (id === modalId) {
            // Reset the form
            const communicationCategoryForm = document.getElementById('communicationCategoryForm');
            if (communicationCategoryForm) {
                communicationCategoryForm.reset();
            }
            
            // Hide message if any
            const communicationCategoryMessage = document.getElementById('communicationCategoryMessage');
            if (communicationCategoryMessage) {
                communicationCategoryMessage.classList.add('hidden');
            }
        }
