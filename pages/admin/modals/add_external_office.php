...
<div id="external-office-add-modal" class="hidden">
    
    <!-- Modal Overlay -->
    <div 
        id="modalOverlay" 
        onclick="closeModal('external-office-add-modal')"
    ...
    <div 
        ...
        <div class="flex items-center justify-between p-5 border-b border-gray-200">
            <h2 class="text-xl font-semibold text-gray-800">Add External Office</h2>
            <button 
                id="closeBtn"
                onclick="closeModal('external-office-add-modal')"
        ...
        <div id="externalOfficeMessage" class="hidden mx-6 mt-4 p-4 rounded-lg text-sm font-medium">
            <div id="externalOfficeMessageContent"></div>
        </div>

        <!-- Modal Body -->
        <form id="externalOfficeForm" class="p-6" method="POST" action="../../includes/actions_admin/add_external_office.php">
            <div class="mb-4">
                <label for="externalOfficeInput" class="block text-sm font-medium text-gray-700 mb-2">
                    External Office Name
                </label>
                <input 
                    type="text" 
                    id="externalOfficeInput"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const modalId = 'external-office-add-modal';
    ...
    window.openModal = function(id) {
        if (id === modalId) {
            // Reset the form
            const externalOfficeForm = document.getElementById('externalOfficeForm');
            if (externalOfficeForm) {
                externalOfficeForm.reset();
            }
            
            // Hide message if any
            const externalOfficeMessage = document.getElementById('externalOfficeMessage');
            if (externalOfficeMessage) {
                externalOfficeMessage.classList.add('hidden');
            }
        }
