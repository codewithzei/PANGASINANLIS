...
<div id="source-type-add-modal" class="hidden">
    
    <!-- Modal Overlay -->
    <div 
        id="modalOverlay" 
        onclick="closeModal('source-type-add-modal')"
    ...
    <div 
        ...
        <div class="flex items-center justify-between p-5 border-b border-gray-200">
            <h2 class="text-xl font-semibold text-gray-800">Add Source Type</h2>
            <button 
                id="closeBtn"
                onclick="closeModal('source-type-add-modal')"
        ...
        <div id="sourceTypeMessage" class="hidden mx-6 mt-4 p-4 rounded-lg text-sm font-medium">
            <div id="sourceTypeMessageContent"></div>
        </div>

        <!-- Modal Body -->
        <form id="sourceTypeForm" class="p-6" method="POST" action="../../includes/actions_admin/add_source_type.php">
            <div class="mb-4">
                <label for="sourceTypeInput" class="block text-sm font-medium text-gray-700 mb-2">
                    Source Type Name
                </label>
                <input 
                    type="text" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const modalId = 'source-type-add-modal';
    ...
    window.openModal = function(id) {
        if (id === modalId) {
            // Reset the form
            const sourceTypeForm = document.getElementById('sourceTypeForm');
            if (sourceTypeForm) {
                sourceTypeForm.reset();
            }
            
            // Hide message if any
            const sourceTypeMessage = document.getElementById('sourceTypeMessage');
            if (sourceTypeMessage) {
                sourceTypeMessage.classList.add('hidden');
            }
        }
