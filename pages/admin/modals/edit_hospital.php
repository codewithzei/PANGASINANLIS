...
<div id="hospital-edit-modal" class="hidden">
    
    <!-- Modal Overlay -->
    <div 
        id="editModalOverlay" 
        onclick="closeModal('hospital-edit-modal')"
    ...
    <div 
        ...
        <div class="flex items-center justify-between p-5 border-b border-gray-200">
            <h2 class="text-xl font-semibold text-gray-800">Edit Hospital</h2>
            <button 
                id="editCloseBtn"
                onclick="closeModal('hospital-edit-modal')"
        ...
        <div id="editHospitalMessage" class="hidden mx-6 mt-4 p-4 rounded-lg text-sm font-medium">
            <div id="editHospitalMessageContent"></div>
        </div>

        <!-- Modal Body -->
        <form id="editHospitalForm" class="p-6" method="POST" action="../../includes/actions_admin/edit_hospital.php">
            <input type="hidden" id="hospitalIdInput" name="hospital_id">
            
            <div class="mb-4">
                <label for="editHospitalInput" class="block text-sm font-medium text-gray-700 mb-2">
                    Hospital Name
                </label>
                <input 
                    type="text" 
                    id="editHospitalInput"
                    name="hospital_name"
...
function editHospital(hospitalId) {
    ...
    fetch('../../includes/fetch_hospitals_data.php?action=getHospital&id=' + hospitalId)
        .then(response => response.json())
        .then(data => {
            if (data.status === 'success') {
                // Populate form fields
                document.getElementById('hospitalIdInput').value = data.data.hospital_id;
                document.getElementById('editHospitalInput').value = data.data.hospital_name;
                ...
                const editHospitalMessage = document.getElementById('editHospitalMessage');
                if (editHospitalMessage) {
                    editHospitalMessage.classList.add('hidden');
                }
                
                // Open the modal
                openModal('hospital-edit-modal');
            } else {
                alert('Error loading hospital data: ' + data.message);
            }
        })
        .catch(error => {
...
function deleteHospital(hospitalId, hospitalName) {
    if (confirm(`Are you sure you want to delete the hospital "${hospitalName}"? This action cannot be undone.`)) {
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = '../../includes/actions_admin/delete_hospital.php';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (editHospitalForm) {
        ...
        editHospitalForm.addEventListener('submit', function(e) {
            const hospitalName = document.getElementById('editHospitalInput').value.trim();
