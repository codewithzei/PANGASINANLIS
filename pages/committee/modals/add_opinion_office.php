<!-- Add Opinion Office Modal Wrapper -->
<div id="opinion-office-add-modal" class="hidden">
    
    <!-- Modal Overlay -->
    <div 
        id="modalOverlay" 
        onclick="closeModal('opinion-office-add-modal')"
    ...
    <div 
        ...
        <div class="flex items-center justify-between p-5 border-b border-gray-200">
            <h2 class="text-xl font-semibold text-gray-800">Add Opinion Office</h2>
            <button 
                id="closeBtn"
                onclick="closeModal('opinion-office-add-modal')"
        ...
        <div id="opinionOfficeMessage" class="hidden mx-6 mt-4 p-4 rounded-lg text-sm font-medium">
            <div id="opinionOfficeMessageContent"></div>
        </div>

        <!-- Modal Body -->
        <form id="opinionOfficeForm" class="p-6" method="POST" action="../../includes/actions_committee/add_opinion_office.php">
    
            <!-- OPINION OFFICE CODE -->
            <div class="mb-4">
                <label for="opinionOfficeCode" class="block text-sm font-medium text-gray-700 mb-2">
                    Opinion Office Code
                </label>
                <input 
                    type="text" 
                    id="opinionOfficeCode"
                    name="opinion_office_code"
                    placeholder="Enter opinion office code"
            ...
            <!-- OPINION OFFICE NAME -->
            <div class="mb-2">
                <label for="opinionOfficeName" class="block text-sm font-medium text-gray-700 mb-2">
                    Opinion Office Name
                </label>
                <input 
                    type="text" 
                    id="opinionOfficeName"
                    name="opinion_office_name"
                    placeholder="Enter opinion office name"
                    class="w-full px-4 py-2.5 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-[#0033A1] focus:border-transparent transition duration-200"
                    required>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const modalId = 'opinion-office-add-modal';
    ...
    window.openModal = function(id) {
        if (id === modalId) {
            // Reset the form
            const opinionOfficeForm = document.getElementById('opinionOfficeForm');
            if (opinionOfficeForm) {
                opinionOfficeForm.reset();
            }
            
            // Hide message if any
            const opinionOfficeMessage = document.getElementById('opinionOfficeMessage');
            if (opinionOfficeMessage) {
                opinionOfficeMessage.classList.add('hidden');
            }
        }
