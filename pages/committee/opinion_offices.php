...
include '../../includes/fetch_opinion_offices_data.php'; // Include the fetch file

$opinionOffices = getAllOpinionOffices();
?>
...
<div class="">
    ...
    <div class="flex-1 md:ml-64 min-h-screen">
        ...
        <?php
        $pageTitle = 'Opinion Offices';
        include '../../components/topbar.php';
        ?>

        <main class="p-6">
            <!-- Global Message Display -->
            <?php if (isset($_SESSION['opinion_office_status'])): ?>
                <div id="globalMessage"
                    ...
                    style="<?php echo $_SESSION['opinion_office_status'] === 'success' ? 'background-color: rgb(220, 252, 231); color: rgb(20, 83, 45); border: 1px solid rgb(167, 243, 208);' : 'background-color: rgb(254, 226, 226); color: rgb(127, 29, 29); border: 1px solid rgb(252, 165, 165);'; ?>">
                    <div class="flex items-center gap-3 px-6 py-4">
                        <?php if ($_SESSION['opinion_office_status'] === 'success'): ?>
                        ...
                        <?php endif; ?>
                        <span class="text-sm font-medium"><?php echo htmlspecialchars($_SESSION['opinion_office_message']); ?></span>
                ...
                unset($_SESSION['opinion_office_status']);
                unset($_SESSION['opinion_office_message']);
                ?>
            <?php endif; ?>
            ...
            <!-- Header with Title -->
                <div class="p-6 border-b border-slate-200">
                    <div class="flex flex-col gap-4">
                        <div>
                            <h2 class="text-xl font-bold text-gray-800">Opinion Offices</h2>
                            <p class="text-sm text-gray-600 mt-1">Manage and configure opinion offices</p>
                        </div>
                        ...
                        <div class="flex items-center gap-3 w-full">
                            ...
                            <button onclick="openModal('opinion-office-add-modal')"
                ...
                <div class="overflow-x-auto">
                    <table id="usersTable" class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-50">
                            <tr>
                                ...
                                <th scope="col"
                                    ...
                                    <div class="flex items-center gap-2">
                                        Office Name
                                        <svg class="h-4 w-4 text-gray-400 sort-icon" data-column="2"
                                            xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                            <path fill-rule="evenodd"
                                                d="M10 3a1 1 0 01.707.293l3 3a1 1 0 01-1.414 1.414L10 5.414 7.707 7.707a1 1 0 01-1.414-1.414l3-3A1 1 0 0110 3zm-3.707 9.293a1 1 0 011.414 0L10 14.586l2.293-2.293a1 1 0 011.414 1.414l-3 3a1 1 0 01-1.414 0l-3-3a1 1 0 010-1.414z"
                                                clip-rule="evenodd" />
                                ...
                                <th scope="col"
                                    ...
                                    <div class="flex items-center gap-2">
                                        Status
                                        <svg class="h-4 w-4 text-gray-400 sort-icon" data-column="3"
                                            xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                            <path fill-rule="evenodd"
                                                d="M10 3a1 1 0 01.707.293l3 3a1 1 0 01-1.414 1.414L10 5.414 7.707 7.707a1 1 0 01-1.414-1.414l3-3A1 1 0 0110 3zm-3.707 9.293a1 1 0 011.414 0L10 14.586l2.293-2.293a1 1 0 011.414 1.414l-3 3a1 1 0 01-1.414 0l-3-3a1 1 0 010-1.414z"
                                                clip-rule="evenodd" />
                        ...
                        <tbody class="bg-white divide-y divide-slate-200" id="tableBody">
                            <?php if (isset($opinionOffices['error'])): ?>
                                <tr>
                                    <td colspan="4" class="px-6 py-8 text-center text-red-600">
                                        <?php echo htmlspecialchars($opinionOffices['error']); ?>
                                    </td>
                                </tr>
                            <?php elseif (empty($opinionOffices)): ?>
                                <!-- Empty state will be added by JavaScript -->
                            <?php else: ?>
                                <?php foreach ($opinionOffices as $office): ?>
                                    <tr class="hover:bg-slate-50 transition-colors duration-150">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                            <?php echo htmlspecialchars($office['opinion_office_id']); ?>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <?php
                                            $statusClass = $office['status'] === 'active'
                                                ? 'bg-green-100 text-green-800'
                                                : 'bg-gray-200 text-gray-800';
                                            ?>
                                            <span
                                                class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full <?php echo $statusClass; ?>">
                                                <?php echo htmlspecialchars(ucfirst($office['status'])); ?>
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex items-center gap-2">
                                                <button onclick="editOpinionOffice(<?php echo $office['opinion_office_id']; ?>)" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-blue-600 bg-blue-100 hover:bg-blue-200 rounded transition duration-150 cursor-pointer">
                                                ...
                                                <button
                                                    onclick="deleteOpinionOffice(<?php echo $office['opinion_office_id']; ?>, '<?php echo htmlspecialchars($office['opinion_office_name']); ?>')"
...
<?php include 'modals/add_opinion_office.php'; ?>
<?php include 'modals/edit_opinion_office.php'; ?>
<?php include 'modals/delete_opinion_office.php'; ?>
