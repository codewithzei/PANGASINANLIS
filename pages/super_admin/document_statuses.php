...
include '../../includes/fetch_doc_stats_data.php'; // Include the fetch file

$docStats = getAllDocStats();
?>
...
<div class="">
    ...
    <div class="flex-1 md:ml-64 min-h-screen">
        ...
        <?php
        $pageTitle = 'Document Statuses';
        include '../../components/topbar.php';
        ?>

        <main class="p-3 sm:p-6">
            <!-- Global Message Display -->
            <?php if (isset($_SESSION['document_status_status'])): ?>
                <div id="globalMessage"
                    ...
                    style="<?php echo $_SESSION['document_status_status'] === 'success' ? 'background-color: rgb(220, 252, 231); color: rgb(20, 83, 45); border: 1px solid rgb(167, 243, 208);' : 'background-color: rgb(254, 226, 226); color: rgb(127, 29, 29); border: 1px solid rgb(252, 165, 165);'; ?>">
                    <div class="flex items-center gap-3 px-6 py-4">
                        <?php if ($_SESSION['document_status_status'] === 'success'): ?>
                        ...
                        <?php endif; ?>
                        <span
                            class="text-sm font-medium"><?php echo htmlspecialchars($_SESSION['document_status_message']); ?></span>
                ...
                unset($_SESSION['document_status_status']);
                unset($_SESSION['document_status_message']);
                ?>
            <?php endif; ?>
            ...
            <!-- Header with Title -->
                <div class="p-6 border-b border-slate-200">
                    <div class="flex flex-col gap-4">
                        <div>
                            <h2 class="text-xl font-bold text-gray-800">Document Statuses</h2>
                            <p class="text-sm text-gray-600 mt-1">Manage and configure document statuses</p>
                        </div>
                        ...
                        <div class="flex items-center gap-3 w-full">
                            ...
                            <button onclick="openModal('doc-stat-add-modal')"
                ...
                <div class="overflow-x-auto">
                    <table id="usersTable" class="min-w-full divide-y divide-slate-200">
                        ...
                        <tbody class="bg-white divide-y divide-slate-200" id="tableBody">
                            <?php if (isset($docStats['error'])): ?>
                                <tr>
                                    <td colspan="4" class="px-6 py-8 text-center text-red-600">
                                        <?php echo htmlspecialchars($docStats['error']); ?>
                                    </td>
                                </tr>
                            <?php elseif (empty($docStats)): ?>
                                <!-- Empty state will be added by JavaScript -->
                            <?php else: ?>
                                <?php foreach ($docStats as $docStat): ?>
                                    <tr class="hover:bg-slate-50 transition-colors duration-150">
                                        <td class="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                            <?php echo htmlspecialchars($docStat['document_status_id']); ?>
                                        </td>
                                        <td class="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-sm text-gray-700">
                                            <?php echo htmlspecialchars($docStat['document_status_name']); ?>
                                        </td>
                                        <td class="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap">
                                            <?php
                                            $statusClass = $docStat['status'] === 'active'
                                                ? 'bg-green-100 text-green-800'
                                                : 'bg-gray-200 text-gray-800';
                                            ?>
                                            <span
                                                class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full <?php echo $statusClass; ?>">
                                                <?php echo htmlspecialchars(ucfirst($docStat['status'])); ?>
                                            </span>
                                        </td>
                                        <td class="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex items-center gap-2">
                                                <button onclick="editDocStat(<?php echo $docStat['document_status_id']; ?>)" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-blue-600 bg-blue-100 hover:bg-blue-200 rounded transition duration-150 cursor-pointer">
                                                ...
                                                <button
                                                    onclick="deleteDocStat(<?php echo $docStat['document_status_id']; ?>, '<?php echo htmlspecialchars($docStat['document_status_name']); ?>')"
...
<?php include 'modals/add_doc_stat.php'; ?>
<?php include 'modals/edit_doc_stat.php'; ?>
<?php include 'modals/delete_doc_stat.php'; ?>
