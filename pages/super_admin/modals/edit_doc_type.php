...
<div id="doc-type-edit-modal" class="hidden">
    
    <!-- Modal Overlay -->
    <div 
        id="editModalOverlay" 
        onclick="closeModal('doc-type-edit-modal')"
    ...
    <div 
        ...
        <div class="flex items-center justify-between p-5 border-b border-gray-200">
            <h2 class="text-xl font-semibold text-gray-800">Edit Document Type</h2>
            <button 
                id="editCloseBtn"
                onclick="closeModal('doc-type-edit-modal')"
        ...
        <div id="editDocTypeMessage" class="hidden mx-6 mt-4 p-4 rounded-lg text-sm font-medium">
            <div id="editDocTypeMessageContent"></div>
        </div>

        <!-- Modal Body -->
        <form id="editDocTypeForm" class="p-6" method="POST" action="../../includes/actions_super_admin/edit_doc_type.php">
            <input type="hidden" id="docTypeIdInput" name="document_type_id">
            
            <div class="mb-4">
                <label for="editDocTypeInput" class="block text-sm font-medium text-gray-700 mb-2">
                    Document Type Name
                </label>
                <input 
                    type="text" 
...
function editDocType(docTypeId) {
    ...
    fetch('../../includes/fetch_doc_types_data.php?action=getDocType&id=' + docTypeId)
        .then(response => response.json())
        .then(data => {
            if (data.status === 'success') {
                // Populate form fields
                document.getElementById('docTypeIdInput').value = data.data.document_type_id;
                document.getElementById('editDocTypeInput').value = data.data.document_type_name;
                ...
                const editDocTypeMessage = document.getElementById('editDocTypeMessage');
                if (editDocTypeMessage) {
                    editDocTypeMessage.classList.add('hidden');
                }
                
                // Open the modal
                openModal('doc-type-edit-modal');
            } else {
                alert('Error loading document type data: ' + data.message);
            }
        })
        .catch(error => {
            console.error('Error fetching document type data:', error);
            alert('Error loading document type data. Please try again.');
        });
}

// Function to delete role
function deleteDocType(docTypeId, docTypeName) {
    if (confirm(`Are you sure you want to delete the document type "${docTypeName}"? This action cannot be undone.`)) {
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = '../../includes/actions_super_admin/delete_doc_type.php';
        
        const input = document.createElement('input');
        input.type = 'hidden';
        input.name = 'document_type_id';
        input.value = docTypeId;
...
document.addEventListener('DOMContentLoaded', function() {
    const editDocTypeForm = document.getElementById('editDocTypeForm');
    
    if (editDocTypeForm) {
        // Optional: Add form submission validation
        editDocTypeForm.addEventListener('submit', function(e) {
            const docTypeName = document.getElementById('editDocTypeInput').value.trim();
