...
include '../../includes/fetch_muni_cities_data.php'; // Include the fetch file

$muniCities = getAllMuniCities();
?>
...
<div class="">
    ...
    <div class="flex-1 md:ml-64 min-h-screen">
        ...
        <?php
        $pageTitle = 'Municipalities/Cities';
        include '../../components/topbar.php';
        ?>

        <main class="p-3 sm:p-6">
            <!-- Global Message Display -->
            <?php if (isset($_SESSION['muni_city_status'])): ?>
                <div id="globalMessage"
                    ...
                    style="<?php echo $_SESSION['muni_city_status'] === 'success' ? 'background-color: rgb(220, 252, 231); color: rgb(20, 83, 45); border: 1px solid rgb(167, 243, 208);' : 'background-color: rgb(254, 226, 226); color: rgb(127, 29, 29); border: 1px solid rgb(252, 165, 165);'; ?>">
                    <div class="flex items-center gap-3 px-6 py-4">
                        <?php if ($_SESSION['muni_city_status'] === 'success'): ?>
                        ...
                        <?php endif; ?>
                        <span
                            class="text-sm font-medium"><?php echo htmlspecialchars($_SESSION['muni_city_message']); ?></span>
                ...
                unset($_SESSION['muni_city_status']);
                unset($_SESSION['muni_city_message']);
                ?>
            <?php endif; ?>
            ...
            <!-- Header with Title -->
                <div class="p-6 border-b border-slate-200">
                    <div class="flex flex-col gap-4">
                        <div>
                            <h2 class="text-xl font-bold text-gray-800">Municipalities & Cities</h2>
                            <p class="text-sm text-gray-600 mt-1">Manage and configure municipal and city</p>
                        </div>
                        ...
                        <div class="flex items-center gap-3 w-full">
                            ...
                            <button onclick="openModal('muni-city-add-modal')"
                            ...
                            <?php if (isset($muniCities['error'])): ?>
                                <tr>
                                    <td colspan="4" class="px-6 py-8 text-center text-red-600">
                                        <?php echo htmlspecialchars($muniCities['error']); ?>
                                    </td>
                                </tr>
                            <?php elseif (empty($muniCities)): ?>
                                ...
                                <?php foreach ($muniCities as $muniCity): ?>
                                    <tr class="hover:bg-slate-50 transition-colors duration-150">
                                        <td class="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                            <?php echo htmlspecialchars($muniCity['muni_city_id']); ?>
                                        </td>
                                        <td class="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-sm text-gray-700">
                                            <?php echo htmlspecialchars($muniCity['muni_city_name']); ?>
                                        </td>
                                        <td class="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap">
                                            <?php
                                            $statusClass = $muniCity['status'] === 'active'
                                                ? 'bg-green-100 text-green-800'
                                                : 'bg-gray-200 text-gray-800';
                                            ?>
                                            <span
                                                class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full <?php echo $statusClass; ?>">
                                                <?php echo htmlspecialchars(ucfirst($muniCity['status'])); ?>
                                            </span>
                                        </td>
                                        <td class="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex items-center gap-2">
                                                <button onclick="editMuniCity(<?php echo $muniCity['muni_city_id']; ?>)" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-blue-600 bg-blue-100 hover:bg-blue-200 rounded transition duration-150 cursor-pointer">
                                                ...
                                                <button
                                                    onclick="deleteMuniCity(<?php echo $muniCity['muni_city_id']; ?>, '<?php echo htmlspecialchars($muniCity['muni_city_name']); ?>')"
...
<?php include 'modals/add_muni_city.php'; ?>
<?php include 'modals/edit_muni_city.php'; ?>
<?php include 'modals/delete_muni_city.php'; ?>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (rows.length === 0) {
            toggleEmptyState('tableBody', true, {
                title: "No data found",
                subtitle: "Add your first municipal/city to get started"
            });
        }
    });
</script>
